...
import { operationRows, operationsSummary } from '../data/mockData'
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="space-y-3 rounded-2xl border border-slate-200 bg-surface p-4 shadow-card">
          ...
          {tabFilters.map((tab, index) => (
            <button
              ...
              className={`rounded-full px-3 py-1.5 text-sm ${
                index === 0 ? 'bg-primary text-white' : 'bg-slate-100 text-slate-700'
              }`}
            >
              {tab}
            </button>
        ...
        <DataTable columns={columns} data={operationRows} />
      </section>
    </div>
  )
}
